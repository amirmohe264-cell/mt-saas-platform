...
class Home extends BaseController
{
    public function index()
    {
        ...
        return $this->view('public/home', ['featuredProducts' => $formattedProducts]);
    }
    ...
    public function products()
    {
        ...
        return $this->view('public/products', ['products' => $allProducts]);
    }
    ...
    public function dashboard()
    {
        ...
        if (!$this->isLoggedIn()) {
            return $this->redirectWithMessage('/login', 'Please login to access your dashboard.', 'error');
        }
        
        // Check if user is customer
        if (!$this->isCustomer()) {
            return $this->redirectWithMessage('/' . $this->getUserRole() . '/dashboard', 'You do not have permission to access this page.', 'error');
        }
    ...
    public function storeDashboard()
{
    ...
    $pendingOrders = $orderModel->where('tenant_id', $tenantId)
                               ->where('order_status', 'pending')
                               ->countAllResults();
    $processingOrders = $orderModel->where('tenant_id', $tenantId)
                                  ->where('order_status', 'processing')
                                  ->countAllResults();
    $completedOrders = $orderModel->where('tenant_id', $tenantId)
                                 ->where('order_status', 'completed')
                                 ->countAllResults();
    
    $totalProducts = $productModel->where('tenant_id', $tenantId)->countAllResults();
    
    // Calculate revenue
    $revenue = $orderModel->selectSum('total_amount')
                         ->where('tenant_id', $tenantId)
                         ->where('order_status', 'completed')
                         ->first()['total_amount'] ?? 0;

    // Recent orders
    $recentOrders = $orderModel->where('tenant_id', $tenantId)
...
>>>>>>> 20cba65f97203a505b07d9170aad5b91ffef4412
    ...
    public function productDetails($slug = null)
    {
        ...
        return $this->view('public/product_details', ['product' => $product]);
    }
}
...
class FixCheckoutErrors extends Migration
{
    public function up()
    {
        ...
        // 3. Add timestamps to order_items (if they exist, skip)
        ...
        // 4. Remove product_name from order_items if it exists
        $this->db->query("ALTER TABLE order_items DROP COLUMN IF EXISTS product_name");
        
        // 5. Add total column to order_items if it doesn't exist
        $this->db->query("ALTER TABLE order_items ADD COLUMN IF NOT EXISTS total DECIMAL(10,2)");
    }
    ...
    public function down()
    {
        ...
        $this->db->query("ALTER TABLE order_items DROP COLUMN total");
    }
}
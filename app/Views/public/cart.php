                                ...
                                <tbody id="cartBody">
                                    <?php foreach ($cartItems as $item): ?>
                                        <tr id="cart-row-<?= $item['product_id'] ?>">
                                            ...
                                            <td class="product-price"><?= number_format($item['price'], 2) ?> ETB</td>
                                            <td>
                                            ...
                                            <td class="product-price"><?= number_format($item['price'] * $item['quantity'], 2) ?> ETB</td>
                                            <td>
                ...
                <div class="col-lg-4">
                    <div class="cart-summary">
                        ...
                        <div class="summary-row">
                            <span>Subtotal</span>
                            <span id="subtotal"><?= number_format($subtotal ?? 0, 2) ?> ETB</span>
                        </div>
                        <div class="summary-row">
                            <span>Shipping</span>
                            <span id="shipping"><?= isset($shipping) && $shipping > 0 ? number_format($shipping, 2) . ' ETB' : 'Free' ?></span>
                        </div>
                        <div class="summary-row">
                            <span>Tax (8%)</span>
                            <span id="tax"><?= number_format($tax ?? 0, 2) ?> ETB</span>
                        </div>
                        <div class="summary-row total">
                            <span>Total</span>
                            <span id="grandTotal"><?= number_format($grandTotal ?? 0, 2) ?> ETB</span>
                        </div>
...
function updateCartTotals() {
    ...
    .then(data => {
        if (data.success) {
            document.getElementById('subtotal').textContent = data.subtotal.toFixed(2) + ' ETB';
            document.getElementById('shipping').textContent = data.shipping > 0 ? data.shipping.toFixed(2) + ' ETB' : 'Free';
            document.getElementById('tax').textContent = data.tax.toFixed(2) + ' ETB';
            document.getElementById('grandTotal').textContent = data.grandTotal.toFixed(2) + ' ETB';

    ...
    <div class="sidebar-card">
        ...
        <ul class="sidebar-menu">
            ...
            <li class="active" onclick="location.href='/orders'" data-tooltip="Orders">
                ...
                <span class="badge-count" id="orderBadge">0</span>
            </li>
        </ul>
        ...
        <ul class="sidebar-menu">
            ...
            <li onclick="location.href='/add-address'" data-tooltip="Add Address">
        ...
        <ul class="sidebar-menu">
            <li onclick="location.href='/settings'" data-tooltip="Settings">
    ...
    <div class="container-fluid px-4 h-100">
        <div class="section-content">
            ...
            <div class="order-count">
                <strong id="orderCountDisplay">0</strong> orders found
            </div>
...
// ==========================================
// 4. ORDER MANAGEMENT (FULLY FUNCTIONAL)
// ==========================================
let orders = [];
...
function getRandomStatus() {
    ...
    const weights = [20, 10, 15, 10, 35, 10]; // Percentage weights
...
function generateSampleOrders() {
    const sampleProducts = [
        ...
        { name: 'Laptop Backpack', price: 49.99, image: '🎒' },
        { name: 'USB-C Cable', price: 19.99, image: '🔌' },
        { name: 'Bluetooth Speaker', price: 89.99, image: '🔊' },
        { name: 'Phone Case', price: 24.99, image: '📱' },
        { name: 'Wireless Mouse', price: 39.99, image: '🖱️' },
        { name: 'Keyboard', price: 59.99, image: '⌨️' },
        { name: 'Monitor Stand', price: 34.99, image: '🖥️' },
        { name: 'Desk Lamp', price: 29.99, image: '💡' }
    ];

    orders = [];
    const now = new Date();
    
    // Generate 15-20 sample orders
...
function renderOrders() {
    ...
    // Update order count
    document.getElementById('orderCountDisplay').textContent = filtered.length;
    ...
    if (filtered.length === 0) {
        container.innerHTML = `
            <div class="text-center py-5">
                ...
                <p class="text-muted">${orders.length === 0 ? 'You haven\'t placed any orders yet.' : 'Try adjusting your filters.'}</p>
                ${orders.length === 0 ? '<a href="/products" class="btn-add-product mt-2"><i class="fas fa-shopping-bag me-2"></i>Start Shopping</a>' : ''}
            </div>
        `;
        return;
    }
            ...
            <div class="order-card">
                <div class="order-header">
                    <div>
                        <div class="order-number">
                            <i class="fas fa-hashtag me-1 text-muted"></i>${order.order_number}
                        </div>
                        <div class="order-date">
                            <i class="fas fa-calendar-alt me-1"></i>${order.date_formatted}
                        </div>
                    </div>
                ...
                <div class="order-footer">
                    <div class="text-muted small">
                        ...
                        ${order.shipping > 0 ? `<span class="mx-2">|</span><span>Shipping: $${order.shipping.toFixed(2)}</span>` : ''}
                    </div>
                    <div>
                        <button class="btn btn-sm btn-outline-success" onclick="viewOrder(${order.id})">
                            <i class="fas fa-eye me-1"></i>View Details
                        </button>
                        ${order.status === 'pending' ? `<button class="btn btn-sm btn-outline-danger" onclick="cancelOrder(${order.id})">
...
function viewOrder(id) {
    ...
    showNotification('info', `📦 Order ${order.order_number}`, `
        <div style="font-size:0.85rem;margin-top:5px;">
            ...
            <strong>Date:</strong> ${order.date_formatted}<br>
            <strong>Items:</strong> ${order.items.reduce((sum, i) => sum + i.quantity, 0)}<br>
            <strong>Subtotal:</strong> $${order.subtotal.toFixed(2)}<br>
            ${order.shipping > 0 ? `<strong>Shipping:</strong> $${order.shipping.toFixed(2)}<br>` : ''}
            <strong>Total:</strong> <span style="font-size:1.1rem;color:#1a2e1a;">$${order.total.toFixed(2)}</span>
        </div>
    `);
}
...
// ==========================================
// 5. ADD SAMPLE ORDER (For testing)
// ==========================================
function addSampleOrder() {
    ...
    const newOrder = {
        ...
        date_formatted: new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
        items: items,
        subtotal: total - shipping,
        shipping: shipping,
        total: total,
    ...
    renderOrders();
    updateOrderBadge();
    showNotification('success', '✅ New Order Created', `Order ${newOrder.order_number} has been created!`);
}

// Add test button (remove in production)
document.addEventListener('DOMContentLoaded', function() {
    // Add a hidden button to create test orders (press Ctrl+Shift+O)

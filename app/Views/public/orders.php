...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        h2, h5, .navbar-brand, .order-total, .order-number {
            font-family: var(--display);
        }

        .notification-container { position: fixed; top: 90px; right: 20px; z-index: 9999; max-width: 380px; width: 100%; }
        .notification-toast { background: #fff; border-radius: 12px; padding: 15px 20px; margin-bottom: 10px; box-shadow: 0 5px 25px rgba(0,0,0,0.15); border-left: 4px solid #10b981; animation: slideInRight 0.4s ease; display: flex; align-items: flex-start; gap: 12px; }
        ...
        .navbar-brand { color: #fff !important; font-weight: 800; font-size: 1.4rem; }
        .navbar-brand i { color: var(--accent); }
        .navbar .nav-link { color: #d1d5db !important; font-weight: 500; transition: 0.3s; font-size: 0.95rem; }
        .navbar .nav-link:hover { color: var(--accent-light) !important; }
        .icon-btn { color: #d1d5db; font-size: 1.1rem; margin: 0 6px; transition: 0.3s; background: none; border: none; }
        .icon-btn:hover { color: var(--accent-light); transform: scale(1.1); }
        .navbar-toggler { border-color: var(--accent); }
        .navbar-toggler-icon { background-image: url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 30 30'%3e%3cpath stroke='rgba(233, 69, 96, 1)' stroke-linecap='round' stroke-miterlimit='10' stroke-width='2' d='M4 7h22M4 15h22M4 23h22'/%3e%3c/svg%3e"); }
        ...
        .sidebar-card .user-avatar { width: 60px; height: 60px; border-radius: 50%; background: var(--accent); color: #fff; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; margin: 0 auto 8px; transition: all 0.3s ease; }
        .sidebar-card .user-name { text-align: center; font-weight: 700; color: var(--brand); font-size: 0.95rem; transition: all 0.3s ease; }
        ...
        .toggle-sidebar-btn:hover { background: #c73652; }
        ...
        .sidebar-menu li:hover { background: #fff5f5; color: var(--accent); }
        .sidebar-menu li.active { background: #fff5f5; color: var(--accent); font-weight: 600; }
        ...
        .sidebar-menu li .badge-count { background: var(--accent); color: #fff; border-radius: 20px; padding: 2px 8px; font-size: 0.6rem; font-weight: 600; margin-left: auto; }
        .sidebar-menu li a { color: inherit; text-decoration: none; display: flex; align-items: center; width: 100%; }

        /* ========================================== */
        /* PAGE HEADER */
        /* ========================================== */
        .page-header { background: var(--surface); color: var(--brand); padding: 12px 0 12px; border-bottom: 1px solid var(--border); height: 70px; display: flex; align-items: center; }
        .page-header h2 { font-weight: 800; color: var(--brand); font-size: 1.4rem; }
        ...
        .section-content::-webkit-scrollbar-thumb { background: var(--accent); border-radius: 4px; }
        .section-content::-webkit-scrollbar-track { background: var(--border); }

        .order-card { background: #fff; border: 1px solid var(--border); border-radius: 12px; padding: 18px 20px; margin-bottom: 15px; transition: all 0.3s ease; }
        .order-card:hover { border-color: var(--accent); box-shadow: 0 2px 12px rgba(233, 69, 96, 0.08); }
        ...
        .order-card .order-number { font-weight: 700; color: var(--brand); font-size: 1rem; }
        .order-card .order-date { color: #888; font-size: 0.85rem; }
        .order-card .order-total { font-weight: 700; color: var(--brand); font-size: 1.1rem; }
        .order-card .order-items { display: flex; flex-wrap: wrap; gap: 10px; margin: 10px 0; }
        .order-card .order-item { background: var(--surface); padding: 6px 12px; border-radius: 6px; font-size: 0.85rem; color: #555; display: inline-flex; align-items: center; gap: 6px; }
        ...
        .btn-add-product { background: var(--accent); color: #fff; border: none; border-radius: 30px; padding: 8px 20px; font-weight: 600; font-size: 0.85rem; transition: 0.3s; text-decoration: none; display: inline-block; }
        .btn-add-product:hover { background: #c73652; color: #fff; }

        .filter-bar { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 20px; }
        .filter-bar .form-select { width: auto; min-width: 150px; font-size: 0.85rem; border-color: var(--border); border-radius: 8px; }
        .filter-bar .form-control { width: auto; min-width: 200px; font-size: 0.85rem; border-color: var(--border); border-radius: 8px; }
        .filter-bar .form-select:focus, .filter-bar .form-control:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(233,69,96,0.1); }
        .filter-bar .btn { font-size: 0.85rem; border-radius: 8px; }
        .filter-bar .btn-success { background: var(--accent); border-color: var(--accent); }
        .filter-bar .btn-success:hover { background: #c73652; border-color: #c73652; }

        .order-count { color: #888; font-size: 0.9rem; margin-bottom: 15px; }
        .order-count strong { color: var(--brand); }

        .empty-state { text-align: center; padding: 60px 20px; }
        .empty-state i { font-size: 4rem; color: var(--border); margin-bottom: 15px; }
        .empty-state h5 { color: var(--brand); font-weight: 700; }
...
<!-- Fixed Sidebar - WITH toggle button INSIDE -->
        ...
        <ul class="sidebar-menu">
            ...
            <li>
                <a href="/logout" data-tooltip="Logout">
                    <i class="fas fa-sign-out-alt" style="color: var(--accent);"></i><span class="menu-text">Logout</span>
                </a>
            </li>
        </ul>
    </div>
...
<section class="page-header">
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                <h2><i class="fas fa-shopping-bag me-2" style="color: var(--accent);"></i>My Orders</h2>
...
function viewOrderDetail(id) {
    ...
    .then(data => {
        ...
        const statusLabel = statusLabels[status] || ucfirst(status);
        const statusIcons = {
            'pending': 'fa-clock',
            'confirmed': 'fa-check-circle',
            'processing': 'fa-spinner',
        ...
        showNotification('info', `📦 Order ${orderNumber}`, `
            <div style="font-size:0.85rem;margin-top:5px;">
                <div class="d-flex justify-content-between border-bottom py-1">
                    <span>Status</span>
                    <span><span class="status-badge ${statusClass}"><i class="fas ${statusIcon} me-1"></i>${statusLabel}</span></span>
                </div>
                <div class="d-flex justify-content-between border-bottom py-1">
                    <span>Date</span>
                    <span>${date}</span>

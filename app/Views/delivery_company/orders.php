<!-- app/Views/delivery_company/orders.php -->
<?php
...
<head>
    ...
    <title>Delivery Orders - ShopEase</title>
...
<section class="page-header">
    <div class="container-fluid px-4">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
                <h2><i class="fas fa-list me-2 text-success"></i>Delivery Orders</h2>
                <nav class="breadcrumb">
                    <a href="/">Home</a>
                    <span class="mx-2">/</span>
                    <a href="/delivery/dashboard">Dashboard</a>
                    <span class="mx-2">/</span>
                    <span class="active">Delivery Orders</span>
                </nav>
            </div>
            <div>
...
<div class="main-content">
    <div class="container-fluid px-4">
        ...
        <div class="mb-3">
            ...
            ?>
                <a href="/delivery/orders?status=<?= $status ?>" 
                   class="filter-btn <?= $currentStatus === $status ? 'active' : '' ?>">
                    <?= $label ?>
                    <?php if ($status !== 'all'): ?>
                        <span class="badge bg-secondary text-white ms-1"><?= $count ?></span>
                    <?php endif; ?>
                </a>
            <?php endforeach; ?>
        </div>
        ...
        <?php if (isset($assignments) && !empty($assignments)): ?>
            <div class="order-count mb-3 text-muted">
                <i class="fas fa-list me-1"></i>Showing <?= count($assignments) ?> order(s)
                <?php if ($currentStatus !== 'all'): ?>
                    <span class="text-muted">(filtered by: <strong><?= ucfirst(str_replace('_', ' ', $currentStatus)) ?></strong>)</span>
                    <a href="/delivery/orders" class="text-success ms-2">Clear filter</a>
                <?php endif; ?>
            </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                        ...
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <span class="status-badge status-<?= str_replace('_', '', $assignment['status']) ?>">
                                <i class="fas <?= $assignment['status'] === 'pending' ? 'fa-clock' : ($assignment['status'] === 'assigned' ? 'fa-user-check' : ($assignment['status'] === 'in_transit' ? 'fa-truck' : ($assignment['status'] === 'delivered' || $assignment['status'] === 'completed' ? 'fa-check-circle' : 'fa-times-circle'))) ?> me-1"></i>
                                <?= ucfirst(str_replace('_', ' ', $assignment['status'])) ?>
                            </span>
                            <span class="fw-bold text-success">$<?= number_format($assignment['order_total'] ?? 0, 2) ?></span>
                            ...
                            <?php endif; ?>
                            <?php if ($assignment['status'] === 'in_transit' || $assignment['status'] === 'picked_up'): ?>
                                <a href="/delivery/orders/complete/<?= $assignment['id'] ?>" class="btn btn-sm btn-success btn-sm-custom" onclick="return confirm('Mark this delivery as completed?')">
        ...
        <?php else: ?>
            <div class="text-center py-5">
                <div class="empty-state">
                    ...
                    <p class="text-muted">
                        <?php if ($currentStatus !== 'all'): ?>
                            No orders with status <strong>"<?= ucfirst(str_replace('_', ' ', $currentStatus)) ?>"</strong> found.
                            <br><a href="/delivery/orders" class="text-success">Clear filter</a>
                        <?php else: ?>
                            Orders will appear here once they are assigned to your company.
                        <?php endif; ?>
                    </p>
                </div>
            </div>
...
<script>
    ...
    console.log('ShopEase Delivery - Orders Page Loaded');
